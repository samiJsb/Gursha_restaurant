...
export default function Settings() {
  ...
  // Form hooks
  const [loungeName, setLoungeName] = useState('Gursha Premium Bole Flagship');
  const [loungeAddr, setLoungeAddr] = useState('Bole Medhanialem Road, Addis Ababa');
  const [loungePhone, setLoungePhone] = useState('+251 911 000 111');
  const [loungeHours, setLoungeHours] = useState('07:00 AM - 11:30 PM');
  ...
  return (
    <div className="min-h-screen bg-[#070709] text-white p-6 md:p-10 font-sans pb-32">
      ...
      <div className="border-b border-white/5 pb-6 mb-10">
        <h1 className="text-3xl font-sans font-black uppercase tracking-tight text-white">SaaS Client Configurations</h1>
        <p className="text-[11px] font-medium text-zinc-500 mt-1 uppercase tracking-widest">
          Configure physical lounges, payment credentials & billing multipliers
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
        ...
        <aside className="lg:col-span-1 space-y-2.5">
          {[
            { id: 'Lounge', label: 'Lounge Hub', icon: Store },
            { id: 'Payments', label: 'API Keyring', icon: Key },
            { id: 'Billing', label: 'Taxes & Delivery', icon: DollarSign },
            ...
            <form onSubmit={triggerSave} className="p-8 bg-[#0B0B0E] border border-white/5 rounded-2xl space-y-6">
              ...
              {saveSuccess && (
                <p className="text-[9px] text-emerald-400 font-bold uppercase tracking-widest">
                  Lounge settings synchronized with global firestore registers.
                </p>
              )}

              <button 
                type="submit" 
                className="px-6 py-3 bg-white text-black text-[9px] font-black uppercase tracking-widest rounded-xl hover:bg-amber-500 transition-all font-bold shadow-lg"
              >
                Sync Lounge Profile
              </button>
            </form>
          ...
          {activeTab === 'Payments' && (
            <form onSubmit={triggerSave} className="p-8 bg-[#0B0B0E] border border-white/5 rounded-2xl space-y-6">
              ...
              <div className="space-y-4 font-mono text-zinc-400 text-[10px]">
                <div className="space-y-1.5">
                  <label className="text-[8px] font-black text-zinc-500 uppercase tracking-widest font-sans">Chapa Public Secret (Bearer Authorizations)</label>
                  <input 
                    type="password" 
                    value={chapaPubKey} 
                ...
                <div className="space-y-1.5">
                  <label className="text-[8px] font-black text-zinc-500 uppercase tracking-widest font-sans">Telebirr AppKey / Superapp Merchant code</label>
                  <input 
                    type="text" 
                    value={telebirrMerchant} 
                    onChange={e => setTelebirrMerchant(e.target.value)}
                    className="w-full bg-zinc-950 border border-white/5 text-[11px] p-3.5 rounded-xl text-white focus:outline-none"
                  />
                </div>

                <div className="space-y-1.5">
                  <label className="text-[8px] font-black text-zinc-500 uppercase tracking-widest font-sans">Commercial Bank of Ethiopia (CBE) Client ID</label>
                  <input 
                    type="text" 
                    value={cbeClientId} 
          ...
          {activeTab === 'Billing' && (
            <form onSubmit={triggerSave} className="p-8 bg-[#0B0B0E] border border-white/5 rounded-2xl space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-1.5">
                  <label className="text-[8px] font-black text-zinc-500 uppercase tracking-widest">Delivery Rate (ETB)</label>
                  <input 
                    type="number" 
                    value={deliveryRate} 
              ...
              <div className="p-4 bg-zinc-900/60 rounded-xl space-y-1 text-zinc-400 text-[10px] font-medium leading-relaxed">
                ...
                <p>Taxation is applied as a net VAT surcharge to total menu prices. Delivery fees are only active for "EXPRESS DELIVERY" logistics routing.</p>
              </div>

              {saveSuccess && (
                <p className="text-[9px] text-emerald-400 font-bold uppercase tracking-widest">
                  Rates updated successfully in REST server memory.
                </p>
              )}
          ...
          {activeTab === 'Security' && (
            <div className="p-8 bg-[#0B0B0E] border border-white/5 rounded-2xl space-y-6">
              ...
              <div className="space-y-4 text-[10px] font-bold text-zinc-400 uppercase tracking-widest font-mono">
                {[
                  { name: 'Kidus Yohannes', email: 'kidus@gursha.com', role: 'ADMIN', color: 'bg-emerald-500/10 text-emerald-400' },
                  { name: 'Almaz Kebede', email: 'almaz.k@gursha.com', role: 'MANAGER', color: 'bg-indigo-500/10 text-indigo-400' },
                  { name: 'Samuel (Sandbox)', email: 'hiring-manager@gurshapremium.com', role: 'STAFF', color: 'bg-zinc-800 text-zinc-400' }
                ].map((staff, idx) => (
